...
export default function Register() {
  ...
  const googleRegister = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      showToast("Registrasi dengan Google berhasil! Mengalihkan...", "success");
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      showToast(error.response?.data?.message || "Registrasi gagal. Coba lagi.", "error");
    }
  };
  ...
  return (
    <div className="auth-page w-full min-h-screen flex" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <Toast toast={toast} onClose={hideToast} />

      <div className="auth-shell flex flex-col md:flex-row w-full" style={{ minHeight: "100vh", background: "#f5f5f5" }}>
        {/* Form Panel */}
        <div
          className="auth-panel auth-panel--form flex flex-col justify-center overflow-y-auto w-full md:w-[46%]"
          style={{ padding: "clamp(20px,4vw,50px) clamp(20px,5vw,80px)", background: "#f8fafc", borderRight: "1px solid rgba(148,163,184,0.1)" }}
        >
          {/* Logo */}
          <div className="flex items-center gap-2 mb-6">
            <img src="/logo.png" alt="Probit Logo" style={{ width: "clamp(32px,3.5vw,50px)", height: "clamp(32px,3.5vw,50px)", objectFit: "contain", borderRadius: "6px" }} />
            <span style={{ fontWeight: 800, fontSize: "clamp(26px,3.5vw,46px)", color: "#0f172a", letterSpacing: "-0.8px" }}>Probit</span>
          </div>

          {/* Card */}
          <div className="auth-card w-full" style={{ maxWidth: "480px" }}>
            <h2 style={{ fontWeight: 700, fontSize: "clamp(20px,2.5vw,32px)", color: "#0f172a", lineHeight: 1.2, marginBottom: "4px", letterSpacing: "-0.5px" }}>
              Create an account
            </h2>
            <p style={{ fontSize: "clamp(12px,1.2vw,16px)", color: "#475569", fontWeight: 500, marginBottom: "14px" }}>
            ...
            <div className="auth-socials flex gap-3 mb-4">
              <button
                ...
                style={{ background: "rgba(255,255,255,0.75)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: "14px", padding: "12px", fontSize: "clamp(13px,1.3vw,16px)", fontWeight: 600, color: "#374151", cursor: "pointer" }}
              >
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <span style={{ fontSize: "clamp(11px,1.1vw,14px)", color: "#64748b", fontWeight: 500 }}>Or register with email</span>
              <div style={{ flex: 1, height: "1px", background: "rgba(148,163,184,0.4)" }} />
            </div>

            {/* Username */}
            <div className="mb-3">
              <label style={{ display: "block", fontSize: "clamp(12px,1.2vw,14px)", fontWeight: 700, color: "#334155", marginBottom: "4px" }}>Username</label>
              <div className="relative flex items-center">
                <User className="absolute" style={{ left: "12px", width: "16px", height: "16px", color: "#64748b" }} />
                <input name="username" value={form.username} onChange={handleChange} type="text" placeholder="Enter your username" style={inputStyle}
            ...
            {/* Email */}
            <div className="mb-3">
              <label style={{ display: "block", fontSize: "clamp(12px,1.2vw,14px)", fontWeight: 700, color: "#334155", marginBottom: "4px" }}>Email Address</label>
              <div className="relative flex items-center">
            ...
            {/* Password */}
            <div className="mb-3">
              <label style={{ display: "block", fontSize: "clamp(12px,1.2vw,14px)", fontWeight: 700, color: "#334155", marginBottom: "4px" }}>Password</label>
              <div className="relative flex items-center">
            ...
            {/* Confirm Password */}
            <div className="mb-3">
              <label style={{ display: "block", fontSize: "clamp(12px,1.2vw,14px)", fontWeight: 700, color: "#334155", marginBottom: "4px" }}>Confirm Password</label>
              <div className="relative flex items-center">
                <Lock className="absolute" style={{ left: "12px", width: "16px", height: "16px", color: "#64748b" }} />
                <input name="confirmPassword" value={form.confirmPassword} onChange={handleChange} type={showConfirmPassword ? "text" : "password"} placeholder="••••••••"
                  style={{
                    ...inputStyle,
                    paddingRight: "40px",
                    borderColor: form.confirmPassword && form.password !== form.confirmPassword ? "#f87171" : "#cbd5e1",
                  }}
                  onFocus={(e) => (e.target.style.borderColor = form.password !== form.confirmPassword ? "#f87171" : "#22c55e")}
                  onBlur={(e) => (e.target.style.borderColor = form.confirmPassword && form.password !== form.confirmPassword ? "#f87171" : "#cbd5e1")}
                />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute flex items-center" style={{ right: "12px", background: "none", border: "none", cursor: "pointer", color: "#64748b", padding: 0 }}>
                  {showConfirmPassword ? <EyeOff style={{ width: "16px", height: "16px" }} /> : <Eye style={{ width: "16px", height: "16px" }} />}
                </button>
              </div>
              {form.confirmPassword && form.password !== form.confirmPassword && (
                <p style={{ fontSize: "11px", color: "#ef4444", marginTop: "4px", paddingLeft: "2px" }}>Password tidak cocok</p>
              )}
            </div>
            ...
            <div className="flex items-start gap-2 mb-4">
              ...
              />
              <label htmlFor="terms" style={{ fontSize: "clamp(11px,1.1vw,14px)", color: "#475569", lineHeight: 1.5, cursor: "pointer" }}>
                I agree to the{" "}
        ...
        {/* Hero Image */}
        <div className="auth-hero relative overflow-auto hidden md:block md:w-[54%]">
